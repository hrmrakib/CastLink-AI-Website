...
function DateCalendarModal({
  ...
  useEffect(() => {
    if (open) setSelectedDates(initialDates);
  }, [open, initialDates]);
...
export default function UpdateTalentPage() {
  ...
  useEffect(() => {
    ...
    // Pre-populate shoot dates
    if (Array.isArray(singleTalent.available_date)) {
      setShootDates(singleTalent.available_date);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Keep existing images the user hasn't removed
      images
